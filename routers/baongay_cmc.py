...
from classes.S3_baongay_cmc import S3_baongay_cmc
...
# async def upload_multi(list: List[upload_multiple_schema]):
    """
    +file_slugs (str): location/new_name mà file sẽ đc lưu, VD: [{"resource_path": "test/a.jpg"}]\n
    ...
    for slug, file in zip(slugs, files):
        result = s3.upload_file(
            bucket_name = config('CMC_BUCKET_NAME'),
            file = file, 
            file_slug=slug,
            public_access=False
        )
